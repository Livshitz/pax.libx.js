...
module.exports = (function () {
  ...
  //#region middlewares:
  mod.middlewares = {}
  mod.middlewares.minify = (options) =>
    streamify(minify(libx.merge({ mangle: false, builtIns: false }, options)))
  ...
  mod.middlewares.if = (condition, middlewareAction) =>
    gulpif(condition, middlewareAction)
  mod.middlewares.ifProd = (middleware) =>
    mod.middlewares.if(mod.config.isProd, middleware)
  ...
  mod.middlewares.vue = (content, file, stylesFile, compilerOptions = {}) => {
    ...
    let style =
      parsed.styles && parsed.styles.length > 0 ? parsed.styles[0].content : ''
    ...
    if (
      ...
      (parsed.template.attrs.lang == 'pug' ||
        parsed.template.attrs.lang == 'jade')
    ) {
    ...
    if (style != null && style != '') {
      try {
        if (
          parsed.styles[0].attrs.lang == 'scss' ||
          parsed.template.attrs.lang == 'sass'
        ) {
    ...
    if (style != null && style.trim() != '') {
      ...
      let newStr =
        `/* ##${componentName} */\n` + style + `\n/* ##${componentName}-end */`
      if (all.indexOf(`/* ##${componentName}`) == -1)
        all = (all || '') + '\n' + newStr + '\n'
      else {
        let reg = new RegExp(
          `\\/\\* ##${componentName} \\*\\/([\\s\\S]\*?)\\/\\* ##${componentName}-end \\*\\/`,
          'sm',
        )
    ...
    if (script.match(/export default ?\{/)) {
      scriptWithTemplate = script.replace(
        /export default ?\{/,
        `$&\n\ttemplate: \`\n${templateEscaped}\`,`,
      )
    ...
    scriptWithTemplate = scriptWithTemplate.replace(
      /\s*?export default ?\{/,
      '\nexports = {',
    )
  ...
  mod.middlewares.usemin = (base) => {
    ...
    return usemin({
      ...
      sitejs: [
        ...
        minify({ mangle: false }).on('error', (uglify) => {
    ...
    var options = {
      ...
      treatChunk: (chunk, options) => {
        if (options.sourcemaps) {
          ...
          // Just rename:
          chunk.contents
            .pipe(source(chunk.path, chunk.base))
            .pipe(rename((f) => (f.extname = '.js')))
            .pipe(gulp.dest(options.sourcemapDest))
        }
        return chunk
  ...
  mod.middlewares.browserify = (_options: any = {}) => {
    ...
    var browserified = through2.obj(function (chunk, enc, callback) {
      if (chunk.isBuffer()) {
        ...
        if (options.babelify)
          bundle.transform(babelify, options.babelifyOptions)
  ...
  mod.middlewares.localize = (libCacheDir, dest, avoidCache) => {
    var transform = async (e, attr, avoidRenameFile?) => {
      ...
      var p =
        (libCacheDir || './') + 'lib-cache/' + (avoidRenameFile ? dir : '')
      ...
      if (avoidCache || !fs.existsSync(f)) {
        ...
        var handler = (data) => {
          if (data == null) throw `Could not find "${src}"!`
        ...
        if (isNetworkResource) {
          const data = await libx.di
            .get('network')
            .httpGet(src, { dataType: '' })
          handler(data)
        } else {
    ...
    return through2.obj(async function (file, encoding, callback) {
      if (file.isBuffer()) {
        ...
        $(
          'script[src][type=""], script[src][type="module"], script[src][type="javascript"], script[src]:not([type])',
        ).each(async (i, e) => {
  ...
  mod.copy = async (_source, dest, middlewares, shouldWatch, _options) => {
    if (middlewares != null && typeof middlewares != 'function')
      throw 'middlewares argument must be an initializator (function)!'
    ...
    libx._.each(middlewares(), (i) => (stream = stream.pipe(i)))
    ...
    if (Array.isArray(_source))
      _source = libx._.map(_source, (i) => i.replace(/^(\!)?\.\//, '$1'))
  ...
  mod.test = async (src, dest) => {
    ...
    gulp
      ...
      .on('error', (err) => libx.log.e('pax.test: --- ERROR: --- ', err))
      .on('end', () => {
  ...
  mod.watch = async (source, dest, middlewares, callback, _options) => {
    if (middlewares != null && typeof middlewares != 'function')
      throw 'middlewares arguments must be an initializator (function)!'
    ...
    mod.watchSimple(
      ...
      async (ev, p) => {
        ...
        libx.log.verbose(
          `pax.watch: File "${p}" ${ev.type}, building to "${dest}"`,
        )
        // options.base = './src'
        // p = path.relative(__dirname, p);
        await mod.copy(
          options.useSourceDir ? source : p,
          dest,
          middlewares,
          false,
          options,
        ...
        libx.log.verbose(
          `pax.watch: ${libx.log.color(
            'Done',
            libx.log.colors.fgGreen,
          )} "${p}"`,
        )
      },
      options,
  ...
  mod.serve = async (path, options, watchPath, watchCallback) => {
    ...
    if (watchPath != null) {
      if (mod.config.devServer.reloadGraceMS == null)
        mod.config.devServer.reloadGraceMS = 1000
      if (mod.config.devServer.reloadDebounceMS == null)
        mod.config.devServer.reloadDebounceMS =
          mod.config.devServer.reloadGraceMS

      libx.log.verbose(
        `pax.serve: starting watch (debounce: ${mod.config.devServer.reloadDebounceMS}ms, grace: ${mod.config.devServer.reloadGraceMS}ms)`,
      )
      var debounce = libx.debounce(
        (path) => {
      ...
      mod.watchSimple(watchPath, (e) => {
        ...
        setTimeout(() => {
          if (mod.config.devServer.livereload == false) return;
          return debounce(e.path);
        }, mod.config.devServer.reloadGraceMS);
      }) //, { cmd: path });
    }
